...
### Added
- **고객사 심볼 라이브러리** — 회사마다 다른 CAD 블록명·심볼을 회사별로
  등록·재사용한다. DXF 블록 **정의 기하의 정규화 지문**과 블록명 별칭 두 키로
  매칭하며, 미인식 블록은 `unknownSymbols`(이름·지문·개수)로 남긴다. 사용자는
  결과 화면에서 아는 심볼만 기기 종류로 확정하고 즉시 재분석할 수 있다. 회사별
  사전과 활성 회사는 브라우저에 보존되어 새로고침·다음 DXF에도 자동 적용되며,
  JSON 반입·반출·삭제를 지원한다. 현행 `fp2`는 내부 엔티티 위치·연결 형상을
  포함해 기존 `fp1`의 동일 개수·외곽비 충돌을 줄였고, 이동·등비 크기·엔티티
  저장 순서는 정규화하며 ARC 각도·타원 비율·polyline bulge·spline 제어점·
  중첩 블록도 구분한다. 같은 지문 또는 블록명이 서로 다른 기기 종류에
  충돌하면 해당 키의 자동 판정을 중단한다. 기존 `fp1` JSON은 계속 읽되 정확한
  블록명으로만 안전하게 이어지고, 별칭 없는 `fp1` 전용 항목과 새 이름의 기하
...
### Fixed
- **일반 AI가 판단을 사용자에게 되넘기던 경로** — `/api/chat`은 책임 전가와 입력을
  되묻는 답변만 같은 공급자·모델로 최대 한 번 교정한다. 교정 재실패·호출 실패·서버
  예산 부족 때는 원답을 노출하지 않고 결정론적 `판단 미완결`로 닫는다. 정상 답변은
  추가 호출하지 않으며, 서버 키 교정 호출은 최초 생성과 별도로 예약·실사용 정산하고
  BYOK·로컬·온프레미스는 서버 예산을 소비하지 않는다. 최종 선택된 답변은 기존
  무출처 수치·출처 필터를 다시 통과한다.
- **모선 구간이 부유 끝점으로 신고되던 것** — `spatial-graph` 의 관계 조립기가
  ...
  *2026-08-13 라이브 실증(33차): 같은 교재 도면 3회에서 거짓 `UNBOUND` 0건,
  `coverage-and-roles` FAIL → HOLD, 종합 pass 2 · hold 7 · **fail 0** — 원장
  최초의 FAIL 0 실행. 라벨 100/100/100(폭 0p), 보정 모델(Gemini)이 아닌
  Terra 에서다. 진짜 신호인 `SELF_LINE_ENDPOINT` 는 남는다.*
