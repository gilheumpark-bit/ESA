...
### Fixed
- **보류가 근거 없는 거절로 보이던 문제** — 제안 화면이 사용자에게 보여주던 것은 `보류 · 원본 근거 1건 · 계산 영수증 0건 · 규칙 1건`이었다. 개수는 근거가 아니다. 규칙 식별자도 화면에 없었고, 무엇을 채우면 판정되는지 알려주는 `requiredInputs`는 아예 렌더링되지 않았다. 실무자 입장에서는 판단 유보가 아니라 그냥 못 하겠다는 말로 읽힌다. `rule-basis.ts`를 추가해 각 규칙의 이름과 "왜 확정하지 못하는가"를 문장으로 제공하고, 화면에 근거 문장 · 판정에 필요한 것 · (기준서 조항인 경우) 원문 확인 링크를 함께 표시한다. ESA 자체 판독 규칙은 `(ESA 판독 규칙)`으로 명시해 기준서 조항인 척하지 않는다. 해석되지 않는 문자열은 그럴듯한 근거 문장으로 포장하지 않고 버린다.
