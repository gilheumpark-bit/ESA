...
for (const width of [1440, 390]) {
  ...
  test(`community search has recoverable failure and reset (${width}px)`, async ({ page }) => {
    ...
    await page.goto('/community'); await expect(page.getByRole('main').getByRole('alert')).toContainText('합성 커뮤니티 장애');
  ...
  test(`administrator role uses server response and paginated safe exports (${width}px)`, async ({ page }) => {
    ...
    await page.goto('/admin'); await page.getByRole('button', { name: '감사 로그', exact: true }).click();
    ...
    const search = page.getByRole('textbox', { name: '감사로그 리소스 검색', exact: true });
