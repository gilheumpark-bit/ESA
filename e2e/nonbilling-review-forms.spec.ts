...
for (const width of [1440, 390]) {
  test(`missing OCR fields can be completed without losing original evidence (${width}px)`, async ({ page }) => {
    ...
    await page.locator('input[type="file"]').setInputFiles({ name: 'synthetic-plate.png', mimeType: 'image/png',
    ...
    await expect(page.getByText(/복수 값·범위·미확정 표기/)).toBeVisible();
    ...
    const input = missing.getByRole('textbox'); await input.fill('원본 확인 제조사'); await input.press('Enter');
