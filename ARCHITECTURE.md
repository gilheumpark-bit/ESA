...
파일 등록
  → 페이지 열거와 예산 계산
  → 전체 이미지와 후보 논리 구획 생성
  → symbols / connections / text / logic 전체 페이지 선행 심사
  → 저확신·복수 후보·고밀도 구획만 역할별 정밀 심사
  → Pxx-S/L/T/R 근거 번호
  → Pxx-A 구획, Pxx-C 경계선, Pxx-U 미확정 끝점
  → 전체선과 선택 구획선 교차검증
  → 전기 공간 그래프 합성 후 coverage 감사
  → 미확정 역할·충돌 위치만 봉인된 이전 결과와 함께 재검사
  → 수량·관계·보호·계산·제안
  → COMPLETE / PARTIAL / HOLD 보고서
```

후보 구획은 페이지 확대 비율과 밀도에 따라 준비하지만 모두 AI에 보내지 않습니다. 전체 페이지 판독에서 저확신·복수 후보·고밀도 근거가 발견된 구획만 정밀 심사하며, 남은 호출 예산이 역할별 상한을 결정합니다. 구획 crop은 경계 누락을 줄이기 위해 겹칠 수 있지만 논리 구획은 겹치지 않습니다. 경계에서 잘린 선은 부품으로 오인하지 않도록 `C` 또는 `U` 번호를 갖습니다. 최종 관계는 구획 결과를 단순 합산하지 않고 전체 도면 그래프에서 다시 연결하고 중복을 제거한 뒤 감사합니다. 감사·충돌에서 재검사 후보가 폭증하면 좌표를 정밀 구획 최대 16개와 전체 원본 1개로 압축하고, 같은 구획에 필요한 역할을 합쳐 공급자 호출의 48개 입력 상한을 넘지 않습니다.
