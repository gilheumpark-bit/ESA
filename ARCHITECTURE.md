...
후보 구획은 페이지 확대 비율과 밀도에 따라 준비하지만 모두 AI에 보내지 않습니다. 전체 페이지 판독에서 저확신·복수 후보·고밀도 근거가 발견된 구획만 정밀 심사하며, 남은 호출 예산이 역할별 상한을 결정합니다. 구획 crop은 경계 누락을 줄이기 위해 겹칠 수 있지만 논리 구획은 겹치지 않습니다. 경계에서 잘린 선은 부품으로 오인하지 않도록 `C` 또는 `U` 번호를 갖습니다. 최종 관계는 구획 결과를 단순 합산하지 않고 전체 도면 그래프에서 다시 연결하고 중복을 제거한 뒤 감사합니다.
