...
// ⚠ 이 데이터는 **아직 화면에 도달하지 않는다** (2026-07-27 실측).
//   `/api/health` 가 `getCertCount().total` 로 개수만 읽고, 과목·조항·
//   판본은 어느 화면도 렌더하지 않는다. 아래 조항 정정은 옳지만 지금은
//   사용자에게 보이지 않는다 — 배선하면 그때부터 보인다.
//
//   `KEC 2021` 표기도 같은 이유로 그대로 뒀다. 화면에 뜨는 판본 도장은
//   조항 레코드(kec-*.ts)와 계산 결과(calculation-execution.ts) 쪽이고
//   그 둘은 정정했다.
//
