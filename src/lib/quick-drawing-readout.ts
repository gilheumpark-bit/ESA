...
export type QuickReadReason = 'QUICK_NOT_VERIFIED' | 'VALUE_NOT_READ' | 'TYPE_UNKNOWN'
  | 'ENDPOINT_UNREAD' | 'SYNTHETIC_JUNCTION';
 ...
 * cannot turn a quick extraction into an independently verified claim. Older
 * DXF results that used load as the fallback are also read as unknown here. */
  ...
  const connections: QuickConnectionRead[] = analysis.connections.map((item: SLDConnection) => {
    ...
    return {
      ...
      relation: !from || !to ? { certainty: 'unread', reason: 'ENDPOINT_UNREAD' }
        : isUnknown(from) || isUnknown(to) ? { certainty: 'ambiguous', reason: 'ENDPOINT_UNREAD' }
