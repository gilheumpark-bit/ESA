 ...
 * **계산하지 말고 가능한 판단과 결론 변경 조건을 분리하라**는 지시다.
 */
export function resolveChatCalculationShortfall(query: string): string | null {
  ...
  return `\n\n계산 요청이지만 검증된 ESA 계산기(${intent.calculatorName ?? intent.calculatorId})를 실행할 입력이 확정되지 않았습니다.\n직접 계산해서 수치를 제시하지 마세요. 주어진 정보로 판단 가능한 범위와 가장 타당한 조건부 결론을 먼저 제시하세요.\n결론을 바꾸는 핵심 입력: ${needed.join(', ') || '없음'}${unread}\n이 목록은 선언형으로 쓰고 사용자에게 질문형으로 되묻지 마세요. 나머지 원리·공식·판단 기준은 수치 없이 계속 설명하세요.${SYMBOLIC_FORMULA_RULE}`;
}
