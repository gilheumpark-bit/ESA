...
export function readSymbolClassification(raw: unknown): SymbolClassification | undefined {
  ...
  if (v.version !== SYMBOL_CLASSIFICATION_VERSION || v.independentVerification !== false
    || !['classified', 'review', 'unread'].includes(String(v.status))
    || !['approved-exact', 'company-library', 'existing-parser', 'family-context', 'human-correction', 'unresolved'].includes(String(v.method))
    || (v.selectedType !== undefined && !types.has(String(v.selectedType)))
  ...
  for (const raw of v.candidates) {
    ...
    if (!types.has(String(c.type)) || typeof c.similarity !== 'number' || !Number.isFinite(c.similarity) || c.similarity < 0 || c.similarity > 1) return undefined;
