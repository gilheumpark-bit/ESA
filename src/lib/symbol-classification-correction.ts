...
export function updateClassificationsAfterCorrection(symbols: SymbolNode[], input: {
  ...
  const sourceTypeChanged = target?.classification?.selectedType !== input.selectedValue.trim();
  ...
  for (const symbol of symbols) {
    ...
    if (symbol.displayId === input.targetDisplayId && input.correctionKind === 'type') {
      ...
      if ((input.correctionKind === 'type' && sourceTypeChanged && ref && previous.referenceKeys.includes(ref))
