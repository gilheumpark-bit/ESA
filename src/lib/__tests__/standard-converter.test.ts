 ...
 *     240.1(단락보호)을 **82% 일치**로 돌려줬다. "232.52".startsWith("232.5")
 *     라는 이유였다. 없는 답보다 틀린 답이 나쁘다.
 *     (그 232.52 는 실재하지 않는 번호였다 — 원문 확인 결과 전압강하는
 *      KEC 232.3.9, 허용전류는 232.5 다. 별도 커밋에서 정정.)
...
describe('조항 매칭', () => {
  ...
  it('마디 경계로 이어지는 것만 하위로 본다', () => {
    // 232.5.1 은 232.5(허용전류)의 진짜 하위 — 상위 항목으로 답해도 된다.
  ...
  it('같은 기준끼리는 그대로 돌려준다', () => {
    const r = convert('KEC', '232.3.9', 'KEC');
    expect(r.toClause).toBe('232.3.9');
...
describe('표 왕복', () => {
  ...
  it('동등 조항 조회도 같은 매칭 규칙을 쓴다', () => {
    // 단락보호 행은 KEC 번호를 비웠다(원문 확인 결과 232.5 는 허용전류라
    // 그 자리에 둘 수 없었고, 대체 번호는 확인 전이라 넣지 않았다).
    // 그래도 다른 기준끼리는 정상 대조돼야 한다.
    const eq = getEquivalentStandards('NEC', '240.1');
    expect(eq.find((e) => e.standard === 'KEC')).toBeUndefined();
    expect(eq.find((e) => e.standard === 'IEC')?.clause).toBe('60364-4-43.4');
  });
});

/** 표에 실린 조항 번호 — 왕복 검사의 입력. */
const TABLE_CLAUSES: Record<'KEC' | 'NEC' | 'IEC' | 'JIS', string[]> = {
  KEC: ['232.1', '232.5', '232.75', '232.10', '232.30', '232.33', '232.8', '232.3.9', '232.11', '232.40'],
