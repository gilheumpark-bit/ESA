...
const MAPPINGS: MappingEntry[] = [
  ...
  },
  {
    ...
    topic_ko: '단락보호',
    // 여기 있던 232.5 는 실제로 허용전류 조항이라 뺐다(원문 확인 2026-07-26).
    // 단락보호의 KEC 조항은 확인 전이라 넣지 않는다 — 틀린 번호보다 없음이 낫다.
    kec: undefined,
    nec: '240.1',
    iec: '60364-4-43.4',
    jis: 'C 60364-4-43.4',
  ...
  },
  {
    ...
    topic_ko: '전선 허용전류',
    // KEC 232.5 = 허용전류(232.5.2 허용전류의 결정). 원문 확인 2026-07-26.
    kec: '232.5',
    nec: '310.16',
    iec: '60364-5-52.523',
    jis: 'C 3005',
  ...
  },
  {
    ...
    topic_ko: '역률 개선',
    // KEC 조항 미상. 여기 있던 232.3.9 는 전압강하 조항이라 뺐다 —
    // 종합설계서(files/전기_전문_AI검색엔진_종합설계서*.md)의 계산기↔규격 표가
    // "전압강하 계산기 | KEC 232.3.9" 로 못박고 있고, 판정층·시험데이터·비교
    // 화면도 전부 그 번호를 쓴다. 설계서는 역률 보정을 220 번대로 적었는데
    // 그 번호대는 KEC 에 없다(2026-07-27 원문 확인) — 확인 없이 넣지 않는다.
    // 확인 전까지 KEC 변환은 "대응 조항 없음" 으로 답한다(틀린 번호보다 낫다).
    kec: undefined,
    nec: '460',
    iec: '60364-5-56.560',
    jis: 'C 4801',
...
function findMapping(standard: StandardCode, clause: string): MappingEntry | undefined {
  ...
  //
  // ① 마디 중간에서 잘린 접두사가 하위로 인정됐다. "232.52" 가 "232.5" 의
  //    하위로 읽혀 "KEC 232.52 → NEC" 가 240.1(단락보호)을 82% 일치로
  //    돌려줬다(실측 2026-07-26). 마디가 끊기지 않았는데 자른 것이다.
