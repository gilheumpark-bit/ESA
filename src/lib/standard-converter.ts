...
const MAPPINGS: MappingEntry[] = [
  ...
  },
  {
    ...
    topic_ko: '역률 개선',
    // KEC 조항 미상. 여기 있던 232.52 는 전압강하 조항이라 뺐다 —
    // 종합설계서(files/전기_전문_AI검색엔진_종합설계서*.md)의 계산기↔규격 표가
    // "전압강하 계산기 | KEC 232.52" 로 못박고 있고, 판정층·시험데이터·비교
...
function findMapping(standard: StandardCode, clause: string): MappingEntry | undefined {
  ...
  //    되는데 실제로는 232.52 의 형제다 — "KEC 232.52 → NEC" 가 240.1(단락보호)을
  //    82% 일치로 돌려줬다(실측 2026-07-26). 표 순서상 232.5 가 232.52 보다 앞이라
  //    먼저 걸린 것이다.
