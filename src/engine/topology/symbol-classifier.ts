...
const special = /\b(?:ATS|ELCB|ELB|RCCB|RCBO|GFCI|SPARE|SPACE|RESERVE|SPECIAL)\b|예비|특수|누전|절체/iu;
...
export function classifyDxfSymbols(components: SLDComponent[], connections: SLDConnection[], options: {
  ...
  for (const component of components) {
    ...
    if (libraryType && component.type === libraryType && shape && !conflict && !special.test(raw)
    ...
    if (component.type !== 'unknown') {
      const reason = conflict ? 'LIBRARY_CONFLICT' : special.test(raw) ? 'SPECIAL_MARKING'
  ...
  for (const feedback of options.library?.feedback ?? []) {
    ...
    if (feedback.status === 'approved' && shape) refs.push({ key: symbolReferenceKey(feedback.source.blockName, feedback.source.fingerprint), type: feedback.deviceType, shape });
  }
  ...
  function rank(shape: SymbolShape) {
    ...
    for (const ref of index.get(shapeTopology(shape)) ?? []) {
      ...
      if (!previous) types.set(ref.type, { type: ref.type, similarity: score, keys: [ref.key] });
  ...
  function repeated(component: SLDComponent, candidate: Ranked): string[] {
    ...
    for (const busId of adjacent) {
      ...
      for (const peerId of adjacency.get(busId) ?? []) {
