...
describe('classification-first context policy', () => {
  ...
  it('shape alone without repeat context remains a candidate', () => {
    ...
    expect(f.candidate.classification).toMatchObject({ status: 'review', selectedType: undefined, reasons: ['SHAPE_MATCH', 'INSUFFICIENT_CONTEXT'] });
  });
  ...
  it.each(['SPARE', 'SPACE', 'ELCB', 'RCCB', 'ATS', '누전', '특수'])('preserves %s as an exception, not a generic repeat', (text) => {
  ...
  it('equally similar different classes do not use first/last wins', () => {
    const f = setup(); f.library.feedback = [approvedFeedback('fuse')]; f.run();
