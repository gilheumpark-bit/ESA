 ...
 * 검증이 입력을 재실행해 claim 전체를 대조하므로, 엔진이 값을 바꿨는데 번호가
 * 그대로면 **옛 영수증이 전부 위조로 보인다.**
 *
 * 올린 뒤에는 `calculation-execution.ts` 의 `ENGINE_VERSION_DRIFT` 가 "우리가
 * 식을 바꿨다" 와 "영수증이 위조다" 를 구분한다.
 */
