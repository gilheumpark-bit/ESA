 ...
 *
 * 조항은 75 개가 등록돼 `/standards` 에 뜨고 저마다 `conditions` 를 갖고
 * 있어서 "이 앱이 75 개 조항을 강제한다" 로 읽힌다. 실측은 다르다 —
 ...
 *
 * 실측 2026-07-28: 조건 보유 75 · 외부에서 param 이 채워지는 것 12 ·
...
describe('조항의 판정 도달', () => {
  ...
  it('사용자 질의가 팀을 통해 비교 판정까지 도달한다', async () => {
    ...
    // 그 수리를 지킨다.
    // 판정 못 낸 기준을 문구가 반드시 말해야 한다. NEC 210.19 는 자리표시자
    // 조항이라 늘 보류인데, 그 사실이 안 보이면 "적합" 이 낙관을 만든다.
    expect(verdict(ok)).toBe('판정된 기준 모두 적합 — 다만 NEC 는 판정 보류(부분 결과)');
    expect(verdict(bad)).toContain('일부 기준 부적합');
  });
  ...
  it('판정에 도달할 수 있는 조항 수가 선언과 같다 — 배선이 늘면 눈에 띄어야 한다', () => {
    ...
    // 총수도 못박는다 — 이 테스트의 존재 이유가 "레지스트리 크기를 강제
    // 범위로 착각하지 말라" 인데, 그 크기가 산문에만 있으면 조용히 어긋난다.
    // 조항을 더하면 여기를 올리고 무엇을 왜 더했는지 적어라.
    //   75: 341.14 피뢰기의 접지 추가(2026-07-28) — 수전설비 피뢰기 조항이
    //       레지스트리에 아예 없었다. 참조·인용용이고 자동 판정은 아니다
    //       (접지저항은 현장 측정값이라 도면에서 안 나온다).
    expect(clauses.size).toBe(75);
