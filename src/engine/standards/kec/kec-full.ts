...
// KEC-232.1 허용전류 ≥ 설계전류 (I_z ≥ I_B, param-vs-param) — 전용 평가자로 판정.
// index.ts KEC_EVALUATORS['KEC-232.1']가 ampacity/designCurrent로 비교한다.
...
// KEC-240.1 과전류 보호: I_B ≤ I_n ≤ I_z (param-vs-param 부등식) — 정적 Condition DSL로 표현 불가.
// 전용 평가자(index.ts KEC_EVALUATORS['KEC-240.1'])가 breakerRating/loadCurrent/wireAmpacity로 판정한다.
...
export const KEC_240_4 = buildArticle('KEC-240.4', '240.4', '과전압 보호 — SPD 설치', [
  cond('spdRating', '>', 0, 'kA', 'SPD(서지보호장치) 설치 — 정격 방전전류 존재'),
]);

// KEC-240.5 차단용량 ≥ 예상 단락전류 (I_cu ≥ I_k, param-vs-param) — 전용 평가자로 판정.
// index.ts KEC_EVALUATORS['KEC-240.5']가 breakingCapacity/prospectiveShortCircuit로 비교한다.
...
export const KEC_351_3 = buildArticle('KEC-351.3', '351.3', '수변전실 — 환기량', [
  cond('ventilationRate', '>', 0, 'm³/h', '수변전실 적정 환기량 확보 — 환기량 존재'),
]);
...
export const KEC_520_5 = buildArticle('KEC-520.5', '520.5', 'ESS — 설치실 환기', [
  cond('ventilationRate', '>', 0, 'm³/h', 'ESS 설치실 적정 환기량 확보 — 환기량 존재'),
]);
