...
// ─── PART 2: (구) KEC 130 전압구분 — 4건 뺐다 2026-07-27 ───────
//
// **130 대는 KEC 에 없다.** 전압 구분은 112 용어 정의에 있고, kec-extended 의
// 112 가 이미 그 값을 담고 있다(교류 1000V 이하 저압 / 1000~7000V 고압 /
// 7000V 초과 특고압).
//
// 하나로 합칠 수도 없었다. 넷 다 같은 `voltage` 파라미터에 1000·1500·50·120
...
// 210.2 조명 분기 1.5mm² 는 바로 위 231.3.1 과 값이 같은 중복이라 뺐다.
// 210.3 동력 분기 2.5mm² 는 내선규정 값이다 — KEC 231.3.1 은 1.5mm² 하나만
// 두고 용도별로 나누지 않는다. 231.3.1 에 합치면 `crossSection >= 1.5` 와
// `>= 2.5` 가 함께 걸려 조명회로까지 2.5mm² 를 요구하게 된다(2026-07-27).
...
// ─── PART 5: (구) KEC 220 부하산정 — 4건 뺐다 2026-07-27 ───────
//
// **220 대는 KEC 에 없다.** 부하 산정은 NEC Article 220 개념이고, 한국에서는
// 내선규정 소관이다. 현행 전문에서 "부하" 로 걸리는 것은 접지(142.4 전기수용가)
// 와 과부하 보호(212.4) 뿐이다.
//
// 뺀 값(재수록하려면 내선규정 원문을 확보해야 한다):
//   주거용 30 VA/m² · 사무실 50 VA/m² · 상업시설 40 VA/m² · 수용률 60%
//
...
// 230.4 접지선 2.5mm² 를 뺐다 — KEC 142.3.1 접지도체는 **6mm²** 이고
// kec-extended 가 그 값을 갖고 있다. 느슨한 쪽을 별도 조항으로 두면
// 사용자가 어느 쪽을 보느냐에 따라 판정이 갈린다(2026-07-27).
...
export const KEC_240_2 = buildArticle('KEC-211.2.4', '211.2.4', '누전차단기의 시설', [
  ...
  // 구 240.3. 감도전류와 동작시간은 같은 조항이 정한다.
  cond('rcdTripTime', '<=', 0.03, 's', '누전차단기 동작시간 0.03초 이하'),
], [
  { articleId: 'NEC-210.8', relation: 'equivalent', note: 'NEC GFCI 보호' },
]);

// 240.3 지락 보호 0.03초를 211.2.4 로 옮겨 합쳤다(아래) 2026-07-27 —
// 현행 240 대는 KEC 에 없고 누전차단기는 211.2.4 다.
...
export const KEC_240_5 = buildArticle('KEC-212.5.5', '212.5.5', '단락보호장치의 특성', [
  ...
  // 구 240.2 직렬 보호(Back-up). 하위 차단기의 부족한 차단용량을 상위가
  // 보완하는 것을 허용하는 것이 이 조항이다.
  cond('backupProtection', '==', 1, 'bool', '하위 차단기 부족한 차단용량을 상위가 보완'),
]);
...
const ALL_EXTENDED_ARTICLES: CodeArticle[] = [
  ...
  // 222.5 저압 가공전선 · 132 절연저항
  KEC_230_2, KEC_230_3,
  // 130 전압구분 · 210.2/210.3 · 220 부하산정 · 230.1/230.4 는 2026-07-27 에
  // 전부 뺐다 — KEC 조항이 아니거나 231.3.1/142.3.1 과 중복이었다.
  // 232 허용전류 확장
  KEC_232_5_2, KEC_232_5_3, KEC_232_5_4,
  // 240 보호
  KEC_240_1, KEC_240_2, KEC_240_4, KEC_240_5,
