...
const DECLARED_RESIDUALS = new Set([
  ...
  '230.4',                                 // → 142.3.1 (이미 사용 중)
  // 410.1/410.2/410.4/410.5 해소 2026-07-27 — KEC 410 편은 **전기철도**다.
  // 접지극 매설깊이·봉길이 → 142.2 · 접지선 → 142.3.2 보호도체 ·
  // 공통접지 → 142.6 공통접지 및 통합접지
  ...
  // 520.1~520.5 해소 2026-07-27 — **520 대는 KEC 에 없다.** 전기저장장치는
  // 511 공통사항 / 512 이차전지다. 충전전압·셀온도 → 511.2.4 이차전지의 시설,
  // 과충전·과방전 보호 → 512.1.4 제어·감시·보호장치. 설치실 환기는
  // 조건이 `>= 0` 이라 항상 PASS 인 자리표시자여서 뺐다.
]);
