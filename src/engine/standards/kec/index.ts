...
function evaluateConditionTree(
  ...
  // 평가할 조건이 하나도 없는 조항은 아래 루프를 그냥 지나쳐 hasFail=false 로
  // 남고, 근거(details)도 matchedConditions 도 빈 채 PASS 가 나간다. 즉 어떤
  // 입력에도 무조건 적합이다. 이것이 BLOCK 이 정의된 이유("근거 없는 판정")
  // 이므로 자리표시자 검사보다 먼저 차단한다. HOLD 가 아니라 BLOCK 인 것은,
  // 입력이 부족한 게 아니라 조항 자체가 판정 근거를 갖고 있지 않기 때문이다.
  if (article.conditions.length === 0) {
    return makeBlock(
      article,
      '조항에 평가 가능한 조건이 없어 근거 없는 PASS 가 된다 — 조항 원문 확인 필요',
    );
  }
