...
const HIGH_VOLTAGE: CodeArticle[] = [
  ...
  /**
   * 피뢰기의 접지 (2026-07-28 추가).
   *
   * 22.9kV·154kV 수전설비에 피뢰기는 필수인데 이 조항이 레지스트리에 없어
   * **피뢰기 판정 자체가 없었다.** 선정 계산기(surge-arrester)는 있는데
   * 시설 적합성을 볼 조항이 비어 있었다.
   *
   * 여기 붙일 조항을 코드 주석 두 곳이 `KEC 153.1.4(서지보호장치)` 라고
   * 적고 있었는데 틀린 조항이다 — 153.1.4 는 내부피뢰시스템의 저압 SPD 다.
   *
   * **근거 등급 E2** — 2 차 출처 2 건이 "고압 및 특고압의 전로에 시설하는
   * 피뢰기 접지저항 값은 10Ω 이하" 로 일치한다. 공식 원문(law.go.kr 행정규칙·
   * kec.kea.kr)은 JS 뷰어라 본문을 못 받아 **원문 대조는 못 했다.**
   * 조항 번호·표제는 KEC 전문 색인(시행 2026.1.5)에 대조돼 있다.
   *
   * 반영 범위: 기본 10Ω 만 건다. 고압가공전선로 피뢰기가 변압기에 근접
   * 시설될 때의 완화(30/75/65Ω)는 넣지 않았다 — 배전 주상설비 조건이라
   * 수전설비 판정 범위 밖이고, 원문 미대조 상태로 예외까지 넣으면
   * 완화 방향(통과하기 쉬운 쪽)으로 틀릴 위험이 크다.
   */
  kec('341.14', '341.14', '피뢰기의 접지', [
    { param: 'arresterGroundingResistance', operator: '<=', value: 10, unit: 'Ω', result: 'PASS', note: '고압·특고압 전로 피뢰기 접지저항 10Ω 이하 (가공전선로 근접 시설 완화 조건은 미반영)' },
  ]),
