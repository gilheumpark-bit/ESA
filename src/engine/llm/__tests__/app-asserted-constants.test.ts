...
describe('앱 근거 상수 — 챗이 말할 수 있어야 한다', () => {
  ...
  describe('통과 — 대상 용어가 곁에 있을 때', () => {
    ...
    /**
     * 접근 한계거리는 **전압을 밝혀야** 통과한다. 0.9m 는 15~37kV 구간 값이고,
     * 어느 전압인지 없이 이격거리를 말하면 다른 전압대에 그대로 적용된다.
     *
     * 전압 자체(22.9)는 목록에 없어 따로 막히므로, 실제 경로처럼 사용자
     * 질문을 신뢰 입력으로 넘긴다.
     */
    it('접근 한계거리 — 전압과 함께면 통과한다', () => {
      const r = filterLLMOutput('22.9kV 접근 한계거리는 0.9m 입니다.', tool, '22.9kV 접근 한계거리?');
      expect(r.filtered).toContain('0.9');
      expect(r.filtered).not.toContain('[미확인]');
    });

    it('접근 한계거리 — 전압 없이 말하면 막는다', () => {
      const r = filterLLMOutput('접근 한계거리는 0.9m 입니다.', tool);
