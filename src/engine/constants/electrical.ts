...
// ═══════════════════════════════════════════════════════════════════════════════
// PART 3 — IEEE 1584-2002 Coefficients
// ═══════════════════════════════════════════════════════════════════════════════
 ...
 *
 * 검증 수준: 공개 문헌 2 곳(ecmag · arcadvisor)에서 계수 단위로 일치 확인.
 * **표준 원문 대조가 아니다** — `arc-flash-known-answer.test.ts` 가 그 수준에서
 * 무엇을 잠그는지 적어 둔다. 값을 바꾸려면 그 검사부터 읽을 것.
 */
export const IEEE_1584_2002 = {
  ...
  /**
   * `E = 4.184·Cf·En·(t/0.2)·(610^x / D^x)` — **E 의 단위는 J/cm² 다.**
   * cal 로 쓰려면 `J_PER_CAL` 로 나눠야 한다. PPE 표는 cal 기준이다.
   *
   * 근거: 같은 표준의 아크 경계식이 `EB = 5.0 J/cm²` 를 쓴다. 경계는 2도 화상
   * 문턱 = 1.2 cal/cm² 이고 5.0 ÷ 4.184 = 1.195 다. 같은 스케일에서 문턱이
   * 5.0 이면 E 는 J 다.
   */
  ENERGY: { J_PER_CAL: 4.184, CF_LV: 1.5, CF_HV: 1.0, REF_DISTANCE_MM: 610, REF_TIME_S: 0.2 },
  /**
   * 2002 Table 4 — 기기 종류·전압대별 거리 지수 `x` 와 통상 전극 간격.
   *
   * **전압대를 반드시 함께 본다.** 1.641 은 저압 MCC·분전반 행이고, 이걸
   * 중고압에 쓰면 과소평가가 된다 — 작업거리가 610mm 보다 멀면(중고압 통상
   * 914mm) 지수가 클수록 에너지가 작아지기 때문이다. 아크플래시에서
   * 과소평가는 보호구 등급을 낮게 잡게 만든다.
   *
   * 개방·케이블은 전압대와 무관하게 2.0. 검증 수준은 위와 같다(2차 문헌).
   *
   * 5~15kV 배전반 간격을 문헌은 153mm 로 적기도 하는데 모델 유효 범위가
   * 13~152mm 라 153 은 범위를 벗어난다. 범위 안에 남는 152 를 쓴다.
   */
  EQUIPMENT_CLASS: {
    /** 0.208~1kV */
    LV: {
      switchgear: { x: 1.473, gap_mm: 32 },
      mcc_panel: { x: 1.641, gap_mm: 25 },
      cable: { x: 2.0, gap_mm: 13 },
      open_air: { x: 2.0, gap_mm: 13 },
    },
    /** >1~15kV (2002 는 1~5kV 와 5~15kV 의 x 가 같다) */
    MV: {
      switchgear: { x: 0.973, gap_mm: 152 },
      mcc_panel: { x: 0.973, gap_mm: 152 },
      cable: { x: 2.0, gap_mm: 13 },
      open_air: { x: 2.0, gap_mm: 13 },
    },
  },
  /** 구판 호환 — 저압 두 행. 새 코드는 `EQUIPMENT_CLASS` 를 쓴다. */
...
/**
 * PPE 등급 경계 (cal/cm²) — NFPA 70E 2018 이후. 값은 각 등급의 최소 내아크
 * 정격이고, 1.2 는 등급이 아니라 2도 화상 경계다(Stoll curve).
 * **Category 0 은 2015 판에서 삭제됐다** — 표는 1 부터 시작한다.
 */
