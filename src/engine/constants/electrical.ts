...
export const IEEE_1584_2002 = {
   ...
   *
   * 앞서 이 자리는 저압 두 행(32·25mm)뿐이었고, 거리 지수는
   * `endsWith('OA') ? 2.0 : 1.641` 로 **전압과 무관하게 1.641** 이었다.
   * 1.641 은 **저압 MCC·분전반** 행이다. 그 결과 중고압에서 두 겹으로
   * 어긋났다(2026-07-28 독립 심사 도메인 좌석 → 2차 문헌 대조로 확정):
   *
   *   ① 거리 지수: >1kV 배전반은 **0.973** 인데 1.641 을 썼다. 작업거리가
   *      610mm 보다 멀면(중고압 통상 914mm) 지수가 클수록 값이 작아진다 —
   *      13.8kV·20kA·0.2s·914mm 에서 **약 −22%**. 과소평가다.
   *   ② 전극 간격: 5~15kV 배전반의 통상 간격은 152mm 인데 저압 기본값
   *      32mm 를 썼다. 간격은 정규화 에너지에 `+0.0011·G` 로 들어가므로
   *      **약 −36%** 가 더 얹힌다.
   *
   * 둘 다 **덜 안전한 방향**이다. 아크플래시에서 과소평가는 보호구 등급을
   * 낮게 잡게 만든다.
   *
   * 검증 수준: 공개 2차 문헌 대조이고 **유료 표준 원문 대조가 아니다**.
   * 개방(open air)·케이블은 전압대와 무관하게 2.0 이다.
   *
   * 5~15kV 배전반의 간격 상한을 문헌은 153mm 로 적기도 하는데, 모델
   * 유효 범위가 13~152mm 라 153 은 스스로 범위를 벗어난다. 152 를 쓴다 —
   * 차이는 무시할 수준이고 범위 안에 남는 쪽이 정직하다.
   */
  EQUIPMENT_CLASS: {
    /** 0.208~1kV */
    LV: {
