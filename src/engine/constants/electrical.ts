...
export const IEEE_1584_2002 = {
   ...
   *
   * 이름을 `UNIT` 에서 `J_PER_CAL` 로 바꾼다. `UNIT` 은 "단위 계수" 라는
   * 뜻 없는 이름이라 읽는 사람이 *이 식의 결과가 이미 cal 이라고* 오해했다 —
   * 실제로 그 오해가 코드에 들어가 있었다(아래 참조).
   *
   * 단위 판별 근거(2026-07-28 독립 심사 지적 → 외부 대조로 확정):
   * 같은 표준의 아크 경계식이 `EB = 5.0 J/cm²` 를 쓴다. 경계는 2도 화상
   * 문턱 = **1.2 cal/cm²** 이고 5.0 J ÷ 4.184 = 1.195 cal 이다. 즉 식 (5) 의
   * E 와 같은 스케일에서 화상 문턱이 5.0 이라는 것은 **E 가 J/cm² 라는 뜻**.
   *
   * 실측 뒷받침: 480V·20kA·0.2s·457mm(교과서 표준 예제)에서 이 식은 41.55 를
   * 낸다. 4.184 로 나누면 9.93 — 업계에서 이 조건의 통상값이다(Category 3).
   * 41.55 cal/cm² 는 40 을 넘어 **"작업 금지"** 로 판정된다. 일상적인 480V
   * 배전반이 작업 금지가 되면 사용자는 이 도구를 끄고 만다.
   */
