 ...
 *
 * 검증 수준(과장 금지): 계수는 공개 문헌 2 곳에서 일치를 확인했고
 * **표준 원문 대조가 아니다.** 2002 판 공개 예제가 없어 known-answer 대신
 * ① 물리 제약 ② 2018 공개 예제와의 거리 ③ 절대 대역으로 잠근다 —
 * `__tests__/arc-flash-known-answer.test.ts` 를 먼저 읽을 것.
 *
 ...
 *
 * 물리 제약: 아크 임피던스가 회로에 더해지므로 **Ia < Ibf** 다. 2002 식은
 * 검증 격자에서 이를 위반하지 않지만 검사는 유지한다 — 계수를 건드리면
 * 그때 잡혀야 한다(교차항·G 항이 빠지면 Ibf 를 넘는 값이 나온다).
 */
function calculateArcingCurrent(
  ...
  // 경계 1kV 는 두 식의 범위에 모두 들어간다. 저압 식은 V→1kV 에서 교차항이
  // 커져 발산하므로(1000V·100kA 에서 Ia/Ibf 1.83) 경계값은 중고압 식으로 보낸다.
  if (voltage_V < 1000) {
 ...
 *
 * **식 (5) 의 결과는 J/cm² 다 — cal 로 바꿔 반환한다.** PPE 표가 cal 기준
 * 이므로 그대로 돌려주면 전 구간 4.184 배 과대가 되고, 일상적인 480V
 * 배전반이 "작업 금지" 로 나온다.
 *
 * `Cf` 는 함체가 아니라 **전압**으로 가른다(<1kV 1.5 · >1kV 1.0).
 */
...
/**
 * 입사 에너지 → 보호구. 두 가지가 인명 문구라 특히 조심할 것:
 *
 *  · **Category 0 은 NFPA 70E 2015 판에서 삭제됐다.** 1.2 cal/cm² 는 등급
 *    경계가 아니라 2도 화상 경계다.
 *  · **합성섬유를 권하면 안 된다** — 130.7(C)(9) 는 315°C 미만에서 녹는
 *    섬유(아세테이트·아크릴·나일론·폴리에스터·폴리에틸렌·폴리프로필렌·
 *    스판덱스)를 단독·혼방 모두 금한다. 녹아 피부에 들러붙어 화상을 키운다.
 */
...
export function calculateArcFlash(input: ArcFlashInput): ArcFlashResult {
  ...
  /**
   * 기기 종류 — 2002 Table 4 의 행. 미기재 시 개방 여부만 `electrodeConfig`
   * 로 보고 나머지는 `switchgear` 다. `electrodeConfig`(VCB/VCBB/HCB)는
   * 2018 판의 전극 배치라 2002 의 기기 분류를 구분해 주지 못한다.
   *
   * 기본을 "값이 크게 나오는 행" 으로 고르지 않는다 — 잘못된 기기의 값을
   * 크게 내는 것은 보수가 아니라 **다른 기기를 계산하는 것**이고, 사용자가
   * 다른 도구와 대조하면 어긋난다. 이 제품의 대상은 수전설비다.
   */
  ...
  steps.push({
    ...
    /**
     * **인용을 130.7(C)(15) 로 되돌리지 말 것.** NFPA 70E 130.5(F) 는 보호구
     * 선정에 두 길을 두고 같은 기기에 병용을 금지하며, **입사 에너지 분석
     * 결과로 130.7(C)(15)(c) 의 등급을 지정하는 것은 허용되지 않는다.**
     *
     * 이 계산기는 입사 에너지 분석을 하므로 정본은 Table 130.5(G) 이고,
     * 산출물은 등급 번호가 아니라 최소 내아크 정격(cal/cm²)이다.
     * `ppeCategory` 는 화면 색·정렬용 내부 밴드이지 NFPA 등급이 아니다.
     */
  ...
  return {
    ...
    warnings: [
      ...
      // 검증 수준을 **화면에** 적는다. 코드 주석에만 두면 값을 보는 사람은 모른다.
      // 이 계산기는 어느 판에도 없는 식으로 표준 이름을 달고 나간 전력이 있다.
      '검증 수준: 식·계수·거리 지수는 공개 문헌으로 대조했고, 결과 절대값은 문헌이'
        + ' 예시로 드는 대역과만 맞췄습니다. **2002 판 원문의 수치 예제와 대조하지'
        + ' 않았습니다.** 보호구 선정은 이 값 하나로 확정하지 말고 전문 소프트웨어'
        + '(ETAP·SKM·EasyPower)나 유자격자 검토와 대조하십시오.',
      ...(gapAssumed
