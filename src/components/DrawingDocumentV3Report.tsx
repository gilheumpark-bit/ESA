    ...
    <section className="min-w-0 overflow-hidden rounded-[10px] border border-[var(--border-default)] bg-[var(--bg-primary)] text-sm shadow-[var(--shadow-card)]">
      <header className="border-b border-[var(--border-default)] bg-[var(--bg-secondary)] p-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-xs font-bold text-[var(--text-secondary)]">● {labelDocumentReadStatus(document.verification.documentStatus)}</span>
          <span className="text-xs text-[var(--text-tertiary)]" title="고정 홀드아웃·현장 라벨·3회 반복·외부 서명 게이트">외부 실증 {document.verification.verified95 ? '완료' : '미확인'}</span>
        </div>
      ...
      <div className="p-3 sm:p-4">
        ...
        {<div hidden={tab !== 'unresolved'}><DrawingReviewQueue key={document.documentHash} document={document}
          selectedDisplayId={selectedDisplayId} onSelectDisplayId={onSelectDisplayId}
          onCorrect={onCorrect} correctingDisplayId={correctingDisplayId} /></div>}
        {tab === 'recommendations' && (document.recommendations.length > 0 ? <ol className="space-y-3">{document.recommendations.map((item) => <li key={item.id} className="rounded-[10px] border border-[var(--border-default)] p-3"><button type="button" disabled={item.relatedDisplayIds.length === 0} onClick={() => item.relatedDisplayIds[0] && onSelectDisplayId?.(item.relatedDisplayIds[0])} className="min-h-11 w-full text-left font-semibold disabled:cursor-default">{item.id} · {item.problem}</button><p className="text-xs"><span className="font-semibold">ESA 판단</span> — {item.aiDecision ?? '이전 분석 결과라 ESA 판단 문장이 없습니다.'}</p><p className="mt-1 text-xs text-[var(--text-secondary)]"><span className="font-semibold">권장 조치</span> — {item.recommendedAction}</p>{describeStandardRefs(item.standardRefs).map((basis) => <p key={basis.label} className="mt-2 border-l-2 border-[var(--border-default)] pl-2 text-xs text-[var(--text-secondary)]"><span className="font-semibold">{basis.label}</span>{basis.internal ? ' (ESA 판독 규칙)' : ''} — {basis.basis}{basis.originUrl ? <> <a href={basis.originUrl} target="_blank" rel="noreferrer" className="underline">원문 확인</a></> : null}</p>)}{item.requiredInputs.length > 0 ? <p className="mt-2 text-xs text-[var(--text-secondary)]"><span className="font-semibold">결론 변경 조건</span> — {item.requiredInputs.join(' · ')}</p> : null}<p className="mt-1 text-xs text-[var(--text-tertiary)]">{labelRecommendationStatus(item.status)} · 원본 근거 {item.evidenceIds.length}건 · 계산 영수증 {item.calcReceiptIds.length}건</p></li>)}</ol> : <p className="py-8 text-center text-xs text-[var(--text-secondary)]">현재 근거로 생성된 제안이 없습니다.</p>)}
      </div>
      <div className="p-3"><DrawingReadingSummary title="정밀 근거의 판독 상태" groups={readSummary.groups} missingGroups={readSummary.missingGroups} humanConfirmed={readSummary.humanConfirmed} causes={readSummary.unresolvedCauses.map((item) => ({ label: labelReadFailureCode(item.code as Parameters<typeof labelReadFailureCode>[0]), count: item.count }))} /></div>
    </section>
  );
}
