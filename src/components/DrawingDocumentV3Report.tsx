...
type Tab = 'counts' | 'devices' | 'relations' | 'values' | 'unresolved' | 'recommendations';

export function DrawingDocumentV3Report({ document, selectedDisplayId, onSelectDisplayId, onCorrect }: DrawingDocumentV3ReportProps) {
  ...
  const tabs: Array<[Tab, string]> = [
    ['counts', '수량'], ['devices', '기기·선로'], ['relations', '관계'], ['values', '정격·계산'], ['unresolved', `미확정 ${document.unresolvedItems.length}`], ['recommendations', '제안'],
  ];
    ...
    <section className="min-w-0 overflow-hidden rounded-[10px] border border-[var(--border-default)] bg-[var(--bg-primary)] text-sm shadow-[var(--shadow-card)]">
      <header className="border-b border-[var(--border-default)] bg-[var(--bg-secondary)] p-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-[11px] font-bold text-[var(--text-secondary)]">● {document.verification.documentStatus}</span>
      ...
      <div className="max-h-[520px] overflow-auto p-4">
        ...
        {tab === 'unresolved' && (document.unresolvedItems.length > 0 ? <ul className="space-y-3">{document.unresolvedItems.map((item) => <li key={item.id} className="rounded-[10px] border border-[var(--border-default)] p-3"><button type="button" className="min-h-11 font-semibold text-[var(--color-primary)]" onClick={() => item.displayId && onSelectDisplayId?.(item.displayId)}>{item.displayId ?? item.id} · {item.code}</button><p className="text-xs text-[var(--text-secondary)]">{item.note}</p>{item.recommendedUpload && <p className="mt-2 text-xs font-medium text-[var(--color-warning)]">재업로드 기준: {item.recommendedUpload.note}</p>}{item.displayId && item.candidates && item.candidates.length > 0 && <div className="mt-2 flex flex-wrap gap-2">{item.candidates.map((candidate) => <button key={candidate} type="button" className="min-h-11 rounded-md border border-[var(--border-default)] px-3 text-xs" onClick={() => onCorrect?.(item.displayId!, candidate, item.candidates ?? [])}>{candidate} 선택</button>)}</div>}{item.displayId && onCorrect && <form className="mt-3 flex gap-2" onSubmit={(event) => {
          event.preventDefault();
          const value = draftCorrections[item.id]?.trim();
          if (value) onCorrect(item.displayId!, value, item.candidates ?? []);
        }}><label htmlFor={`correction-${item.id}`} className="sr-only">{item.displayId} 직접 수정값</label><input id={`correction-${item.id}`} name={`correction-${item.id}`} value={draftCorrections[item.id] ?? ''} onChange={(event) => setDraftCorrections((current) => ({ ...current, [item.id]: event.target.value }))} placeholder="직접 수정값 입력" className="min-h-11 min-w-0 flex-1 rounded-md border border-[var(--border-default)] bg-[var(--bg-primary)] px-3 text-sm" /><button type="submit" disabled={!draftCorrections[item.id]?.trim()} className="min-h-11 rounded-md bg-[var(--color-primary)] px-3 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">수정 반영</button></form>}</li>)}</ul> : <p className="py-8 text-center text-xs text-[var(--text-secondary)]">사용자 확인이 필요한 항목이 없습니다.</p>)}
        {tab === 'recommendations' && (document.recommendations.length > 0 ? <ol className="space-y-3">{document.recommendations.map((item) => <li key={item.id} className="rounded-[10px] border border-[var(--border-default)] p-3"><button type="button" disabled={item.relatedDisplayIds.length === 0} onClick={() => item.relatedDisplayIds[0] && onSelectDisplayId?.(item.relatedDisplayIds[0])} className="min-h-11 w-full text-left font-semibold disabled:cursor-default">{item.id} · {item.problem}</button><p className="text-xs text-[var(--text-secondary)]">{item.recommendedAction}</p><p className="mt-1 text-[11px] text-[var(--text-tertiary)]">{item.status} · 원본 근거 {item.evidenceIds.length}건 · 계산 영수증 {item.calcReceiptIds.length}건 · 규칙 {item.standardRefs.length}건</p></li>)}</ol> : <p className="py-8 text-center text-xs text-[var(--text-secondary)]">현재 근거로 생성된 제안이 없습니다.</p>)}
      </div>
    </section>
  );
}
