...
const control = 'min-h-11 max-w-full rounded-lg border border-[var(--border-hover)] bg-[var(--bg-primary)] px-3 py-2 text-sm [overflow-wrap:anywhere] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--color-primary)] disabled:opacity-50';
export function DrawingWorkProductPanel({ document, onSelect, onReview }: {
  ...
  return <section aria-label="AX 기기표" className="min-w-0 space-y-4">
    ...
    <div className="min-w-0 max-w-full overflow-x-auto"><table className="w-full table-fixed text-right text-xs tabular-nums">
      <caption className="mb-2 text-left text-sm font-semibold">분류별 검출 기호 집계 · 물리 대수 아님</caption>
      <thead><tr className="border-b border-[var(--border-default)]"><th scope="col" className="w-[31%] p-2 text-left">종류</th><th scope="col" className="break-keep p-2">기존 판독</th><th scope="col" className="break-keep p-2">자동 분류</th><th scope="col" className="break-keep p-2">초안 합계</th></tr></thead>
      <tbody>{product.groups.map((group) => <tr key={group.type} className="border-b border-[var(--border-default)]"><th scope="row" className="p-2 text-left font-medium [overflow-wrap:anywhere]">{group.type}</th><td className="p-2">{group.confirmedReadings}</td><td className="p-2">{group.automaticClassifications}</td><td className="p-2">{group.usableOccurrences}</td></tr>)}</tbody>
    </table></div>
    ...
    <ul aria-label="기기표 항목" className="space-y-3">{rows.slice(0, limit).map((row, index) => <li key={`${row.symbolId}:${index}`} className="min-w-0 rounded-xl border border-[var(--border-default)] p-3">
      <p className="text-sm font-semibold [overflow-wrap:anywhere]">{row.displayId} · {row.interpretedType ?? '종류 미확정'}{row.label ? ` · ${row.label}` : ''}</p>
