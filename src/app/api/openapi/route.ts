  ...
  paths: {
    ...
    '/calculate': {
      post: {
        ...
        // 실제 라우트가 내는 상태를 전부 적는다. 앞서 200·400·404 뿐이었고
        // **이 배치의 주제인 422 가 빠져 있었다** — 같은 파일 위 주석은
        // "전에는 … 422 였다(실측)" 라고 쓰면서 응답 목록은 안 고쳤다
        // (2026-07-28 독립 심사 백엔드 좌석). 통합하는 쪽은 이 표를 보고
        // 오류 처리를 짜므로, 없는 상태가 오면 그쪽이 깨진다.
        responses: {
          200: { description: '계산 결과 + 영수증' },
          400: { description: '요청 형식 오류 (JSON 파싱 실패·본문 초과 등)' },
          401: { description: '인증 필요' },
          403: { description: '권한 없음 (요금제·출처 제한)' },
          404: { description: '계산기 미발견' },
          422: { description: '입력 값 거부 — `error.field` 에 문제된 입력 칸 이름이 온다' },
