...
async function GET__impl(request: NextRequest) {
  ...
  try {
    ...
    if (error || data === null || count === null) return json({ success: false, error: { message: '감사로그 조회에 실패했습니다. 빈 목록으로 처리하지 않았습니다.' } }, 503);
    return json({ success: true, data: { entries: data.map((row) => ({ id: row.id, userId: row.user_id ?? '',
      ...
      totalPages: Math.max(1, Math.ceil(count / pageSize)), scope: 'system-admin' } });
