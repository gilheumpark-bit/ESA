...
> 기준일: 2026-08-26 · ESVA v0.2.0 · 제품 코드 기준 `3c9172f`
...
| 상태 | 의미 | 통합 시 처리 |
|---|---|---|
| `400` | 요청 형식 오류(JSON 파싱 실패·본문 초과) | 요청을 고쳐 재시도 |
| `401` · `403` | 인증 필요 · 권한 없음(요금제·출처 제한) | 재시도해도 같음 |
| `404` | 계산기 미발견 | `calculatorId` 확인 |
| `422` | **입력 값 거부** — 사용자가 고칠 수 있다 | `error.field` 가 문제된 입력 칸 이름이다. 그 칸을 짚어 보여줄 것 |
| `429` | 요청 한도 초과 | 백오프 후 재시도 |
| `500` | 서버 내부 오류 | 재시도하지 말고 보고. 이 상태는 **우리 데이터·상태가 깨진 것**이고 알람 대상이다 |

**422 와 500 을 뭉뚱그리지 마십시오.** 422 는 입력을 고치면 풀리고, 500 은
고칠 수 없습니다. 계산 경로는 이 둘을 분리해 던지므로 통합하는 쪽도
분리해 다뤄야 합니다.

```json
{
  "success": false,
  "error": { "code": "ESVA-4010", "message": "…", "field": "cableSize" }
}
```

`field` 는 폼에 실재하는 칸 이름입니다 — 내부 이름을 노출하지 않습니다.
기계가 읽는 선언은 `GET /api/openapi` 가 정본이며,
`spec-matches-reality.test.ts` 가 라우트가 내는 상태와 그 선언을 대조합니다.
