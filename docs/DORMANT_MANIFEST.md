...
> 2026-09-09 점검. 활성 production 코드의 미해결 부채는
> [기술부채 대장](TECHNICAL_DEBT.md)에서 추적하고, 이 문서는 의도적으로 꺼 둔
> 기능만 다룬다. 이전 전수 점검에서는 화면·호출처가 없고 성공할 수 없던
...
| `agent/drawing/role-runner.ts` | DORMANT(호환/시험) | production 호출은 없고 barrel 재수출과 Google transport 회귀에서 사용한다. 실제 production은 drawing-council → analyzeDrawingRole이다. | 분석/호환성 | 재수출 계약·회귀를 production 경로로 이관한 뒤 제거 여부 결정 | 호출 경로 통합 시 |
...
| 요금제 일일 한도(`calcPerDay`·`aiChatPerDay`) | DORMANT(집행 없음) | `TIER_LIMITS` 표에 free 10회/5회, pro 무제한/100회 … 로 적혀 있으나 **세는 곳이 없다**. 이를 읽던 `checkDailyUsage` 는 호출처 0 이었고 2026-07-29 에 지웠다 — 서명이 `(tier, feature, currentUsage)` 라 사용량을 호출자가 주게 돼 있어, 정작 어려운 부분(사용자별 일일 카운터 저장소)이 없는 채로 다 있는 것처럼 보였다. 숫자 자체는 제품 결정이라 표에 남긴다. **어느 화면에도 광고되지 않으므로 깨진 약속은 아니다** — 설정 화면은 플랜 배지와 결제 버튼만 보여 준다. 지금 실제로 도는 비용 방어는 `/api/chat` 의 IP 당 일일 토큰 예산(서버 키 요청 한정)과 `checkRateLimit` 이다 | 요금제/결제 | 사용자별 일일 카운터 저장소(Supabase/Firestore)와 초기화 시각·시간대 정책이 정해질 때. 그때 UI 에 잔여 횟수를 함께 띄울지도 같이 정할 것 | 유료 플랜 출시 착수 시 |
