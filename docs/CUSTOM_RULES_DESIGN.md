...
두 가지 작성 방식을 모두 지원하며, `result`가 방향을 정한다:

| 방식 | 예 | 의미 |
|---|---|---|
| **PASS형** — 성립해야 적합 | `voltageDropPercent <= 2.5, result:"PASS"` | 충족 = 적합 · 미충족 = 위반 |
| **FAIL형** — 성립하면 위반 | `voltageDropPercent > 2.5, result:"FAIL"` | 충족 = 위반 · 미충족 = 적합 |

> 이력: 최초 구현은 `result`를 읽지 않고 충족=적합으로만 처리해 FAIL형 작성 시
> 판정이 완전히 반전됐다(실위반 5% → PASS). 독립 심사가 실행 재현으로 발각,
> 수리 후 네 사분면 전부 테스트로 잠금.
...
## 독립 심사 결과 (2026-07-20 · fresh-context IND-1)

구현 커밋(91ea1cb)에 대해 코드만 보는 독립 심사를 실행 — **CRITICAL 2·MAJOR 3·
MINOR 4 발견, 전량 수리**. CRITICAL 두 건은 심사자가 실제 실행으로 재현했다:

1. **result 필드 반전** — FAIL형 조건 작성 시 실위반→PASS·실적합→FAIL. 수리:
   평가가 result 방향을 반영, 4사분면 테스트 잠금, 라이브 재확증.
2. **병합 dedup이 인스턴스 FAIL 삼킴** — 같은 조항 키에서 먼저 온 PASS가 이겨
   변압기 2대 중 1대의 위반이 병합표·verdict에서 소실. KEC 232.52 등 기존
   행도 동일 결함(사전 존재). 수리: 최악 판정 보존(BLOCK>FAIL>HOLD>PASS),
   라이브에서 verdict FAIL/F 반영 확증.

MAJOR: 바이트 캡이 UTF-16 길이 기준(한글 3배 우회)→`Buffer.byteLength` ·
`as File` 무검증(문자열 파트가 크기 캡 우회)→타입 검사+400 · 미평가를 응답이
숨김→`evaluated`/`evaluatedBy` 필드. MINOR: appliesTo 캡 · unit 침묵 절단 ·
JSON 바디 rules 무시→400 안내 · 다중 위반 첫 건만 노출→3건 나열.
