...
| 이미지 도면 분석 | `POST /api/sld`, `POST /api/team-review` | SLD 업로드와 Studio | Sharp 방향 정규화, 전체·구획·업스케일 변형 뒤 역할별 VLM 판독 | 팀 보고서 Supabase 저장 | `GET /api/reports/[id]` | `/tools/sld`, `/report/[id]` | 형식·크기·키 검증, 역할 실패·누락·불일치 HOLD | Vision 크롭·병합, 역할별 호출, 봉투 무결성 | wired |
...
| SLD 원본 격리 심사와 전기 합성 | `src/agent/teams/sld-team.ts` | 오케스트레이터의 `TEAM-SLD` | `symbols`·`connections`·`text`·`logic` 봉투를 정규화하고 불변식·논리 충돌·계산 입력을 합성 | 보고서의 `drawingSynthesis`와 해시 | 합의 단계가 같은 drawing hash와 필수 역할을 재검증 | `/report/[id]`의 HOLD·FAIL·추천·계산 | 역할 누락, 출처 alias 모호성, 전압·경로·보호·접지 불명확 시 fail-closed | 독립 심사, normalizer, invariants, router, synthesis, consensus integrity | wired |
| SLD 원본 근거 보고서 | SLD 업로드와 `drawing-asset-store` | `/tools/sld` 분석 완료 | 원본 바이트는 IndexedDB, 보고서에는 해시·기호·선·수량·근거 번호만 기록 | 브라우저 로컬 원본 + Supabase 보고서 | 원본 SHA-256 일치 뒤 object URL 재생성 | `/report/[id]` 오버레이와 표의 양방향 선택 | 원본 부재·해시 불일치·세션 부재 시 가짜 도면·점수 없이 안내 | asset store, report integrity, evidence numbering, CSP, Playwright | wired |
| SLD 95% 주장 게이트 | `scripts/sld-golden-gate.mjs` | receipt 생성과 `verifyCurrentGoldenGate` | 정확한 manifest 집합, labels·predictions hash, evaluator version, Ed25519 서명을 검증 | 서명된 receipt 파일 | 새 프로세스에서 최신 receipt와 현재 파일을 다시 hash | 보고서의 `verified95` 표시 조건 | 실도면 독립 라벨·키·예측·완전한 데이터셋 중 하나라도 없으면 exit 1/HOLD | golden gate·metrics·exact dataset coverage 적대 테스트 | partial |
