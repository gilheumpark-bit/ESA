...
## 무엇을 고쳤나 (2026-07-28 후속)

임계를 옮기는 것은 여전히 못 한다. 대신 **임계가 필요 없는 신호**가 이 표
안에 있었다.

| 연결 | 실행 | 차단기 오차 |
|---|---|---|
| **0** | p40 raster · p40 heavy · p14 light · p14 heavy | 3.50 · 7.00 · 2.33 · 4.00 배 |
| 있음 | p14 raster · p5 raster · p5 light · p5 heavy | 1.00 · 1.29 · 1.13 · 1.29 배 |

두 무리 사이가 비어 있다(1.29 ↔ 2.33). 그러나 **이 상관이 근거의 본체가
아니다** — n=8 로 분류기를 세운 게 아니다. 근거는 이것이다: *연결이 0 이면
계통 판정이 불가하다는 것은 출력에 대한 사실*이고, 화질 판정이 아니다.

고친 것 둘:

1. **검토 규칙 5** (`circuit-review.ts`) — 부품 여럿 · 연결 0 · 스케줄 표
   아님이면 `DATA-GAP/UNKNOWN` 을 낸다: "부품은 읽혔는데 연결선을 하나도
   읽지 못했다 — 목록으로만 쓰고 계통 판단은 원본에서." 규칙 4 는 같은
   상황을 "케이블 미결속 24/24" 로만 적었고, 그건 케이블 정보가 모자란
   것으로 읽히지 계통을 통째로 못 읽은 것으로는 읽히지 않는다.

2. **계통 판독 노출** (`ReviewReportPanel.tsx`) — 사가의 validate 단계가
   세는 `nodeCount·edgeCount·isolatedNodes·connectedComponents` 는 응답에
   실려 나가는데 **화면이 통째로 버리고 있었다**(src 전체 소비처 0). 검토
   패널에 한 줄로 싣는다. 고립 노드를 하나씩 나열하지 않는다 — 연결 0 이면
   전부 고립이라 그 길이가 신호를 덮는다(§3 신호 압축).

**이 신호가 못 잡는 것**: 정격 오독. p5 scan-heavy 는 연결 12 개가 정상적으로
잡힌 채로 500kVA 를 300·1000kVA 로 읽었다. 그쪽은 `TR-MAIN-CURRENT` 의
2차전류 대조가 담당하는데, 그 대조도 **과대 방향만** 잡는다(페이지 최대
트립보다 I₂ 가 클 때). 과소 방향은 다중 TR 페이지에서 정상 도면을
오탐하므로 페이지 최대 트립으로는 판정할 수 없다 — 주차단기 식별이
선행돼야 하고, 그건 연결이 필요하다.
...
- **교보재 확대** — 실제 사용자 업로드 분포에 가까운 스캔 표본. 지금 셋은
  ...
  대표하지 못한다.
- ~~**가드의 성격 결정**~~ — 연결 0 건에 대해서는 **결정했다**: 차단하지
  않고 UNKNOWN 으로 선언한다(위 규칙 5). 리포의 기존 관례가 낙관 PASS 대신
  UNKNOWN 이고(`gate:pdf` R15), 부품 목록 자체는 여전히 쓸모가 있기
  때문이다. 남는 것은 **화질 기반** 가드의 성격이며, 그건 임계가 필요하고
  임계는 교보재가 필요하다.
- ~~**부품 단위 confidence**~~ — 처음엔 이걸 선행 조건으로 적었으나
  **철회한다.** 부품별 확신도도 결국 **같은 모델의 자기신고**다. 위에서 본
  대로 1000kVA 로 틀린 실행이 문서 confidence 0.9 를 냈다 — 자기채점은
  자기 오독을 가리키지 못한다(§2.3 닫힌 순환). 단위를 잘게 쪼갠다고
  오라클이 되지 않는다.

  대신 **도면 안의 다른 값과 맞춰 보는 것**이 이 자리에서 쓸 수 있는 유일한
  독립 근거다. 2026-07-28 후속 커밋에서 `TR-MAIN-CURRENT` 에 그 대조를
  붙였다 — 2차 주차단기 트립이 정격 2차전류를 상회하는지 본다. 실측 오독
  사례(1000kVA → I₂ 1519A vs 800AT)가 이 대조에 걸린다. 다만 **과대 오독만**
  잡는다(300kVA 과소 오독은 456A 라 800AT 안에 들어온다) — 과소 방향의
  독립 근거는 아직 없다.
