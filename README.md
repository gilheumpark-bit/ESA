...
필수 입력이 없거나 파서 확신도가 낮으면 계산기를 실행하지 않습니다. 이때 AI는 역질문으로 판단을 넘기지 않고 현재 근거의 조건부 결론과 `결론을 바꾸는 핵심 입력`을 선언형으로 제시한 뒤 나머지 설명을 계속합니다. 임의 수치로 적합 판정을 만들지는 않습니다. `npm run gate:chat-live`는 production 서버에서 전압강하 계산을 실제 실행하고, 같은 영수증이 모델 입력과 사용자 응답에 전달되는지 검사합니다.
