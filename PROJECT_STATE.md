...
- **전압강하가 낙관 방향으로 치우친다 — 설계 판단 대기.** `R = ρ×1000/A`의 ρ가 20°C 저항률이고 운전온도 보정이 없다(70/90°C 도체는 저항이 15~25% 높으므로 전압강하가 그만큼 과소평가된다). 리액턴스도 굵기 무관 `0.08 Ω/km` 고정이다. 둘 다 계산기 헤더에 명시돼 있어 숨긴 것은 아니지만 방향이 낙관이다. `TEMPERATURE_COEFF_CU`와 `global/temp-correction.ts`가 이미 있으나 본 경로가 쓰지 않는다. 공식을 바꾸는 것은 전기 실무 판단이므로 임의로 넣지 않았다.
