...
status: active
baselineBranch: main
codeBaselineCommit: 11d4b025429680124d0fbfe13bbf6fa31a901226
updatedAt: 2026-08-09T05:40:10+09:00
trigger: commits
...
- 현재 코드 기준선은 `11d4b025429680124d0fbfe13bbf6fa31a901226`이다. 기존 17개 라이브 영수증은 호출 당시 동일 dirty snapshot `f70da7f6…`에 결박돼 있고, 2026-08-08 전역 선행·선택 구획 구조의 초급 단발 원본 영수증은 저장소에 남지 않았다. 생성된 `.next/`, `test-results/`, 검증용 작업 JSON과 브라우저 임시 업로드는 Git에 포함하지 않는다.
...
2. `11d4b02`의 전역 선행·선택 구획 구조로 GPT 중급·고급을 같은 snapshot에서 반복 실행해 저장소 밖 원본 영수증과 집계 영수증을 보존한다. 라벨 점수는 고급에서 조립기 변경을 탐지하지 못하므로 정답 없이 재는 조립 지표(`scripts/measure-assembly-quality.mjs`)와 함께 읽는다.
