...
- 페이지 확대 비율과 선·기호 밀도에 따라 2×2·3×3·4×4 구획을 선택한다. 기호 심사가 하나 이상 살아 있는 래스터 페이지에서만 결정론적 직선 검출을 보조 근거로 사용하며, 문자·기기 내부 선과 고립된 짧은 선은 제외한다.
...
- 현재 코드 기준선은 `c82f9f8ac2cc5114aa264a67ffdccbc5c8b35a6c`이다(이전 기록 `0d475fc1961a3f9daab8687ae54551702590f037`). 17개 라이브 영수증은 호출 당시 동일 dirty snapshot `f70da7f6…`에, 현재 후보 게이트 재채점은 clean `0d475fc`에 결박돼 있다. 생성된 `.next/`, `test-results/`, 검증용 작업 JSON과 브라우저 임시 업로드는 Git에 포함하지 않았다.
...
2. ~~공급자·모델별 3회 실행~~ — 수행함(원장 10~21차). 후속은 **지표를 먼저 고치는 것**이다: 라벨 점수는 고급에서 조립기 변경을 탐지하지 못하므로 정답 없이 재는 조립 지표(`scripts/measure-assembly-quality.mjs`)와 함께 읽는다.
